...
impl Solution {
    fn can_swim_through(grid: &Vec<Vec<i32>>, n: usize, expected_time: i32) -> bool {
        let l = expected_time;
        let grid: Vec<Vec<i32>> = grid
            .iter()
            .map(|row| row.iter().map(|&val| val.max(expected_time)).collect())
            .collect();
        println!("{expected_time} {grid:?}");
        ...
        while let Some((x, y)) = stack.pop() {
            ...
            for (n_x, n_y) in vec![
                ...
                if n_x < n && n_y < n && !is_visited[n_x][n_y] && grid[n_x][n_y] <= l {
                    stack.push((n_x, n_y));
    ...
    pub fn swim_in_water(grid: Vec<Vec<i32>>) -> i32 {
        ...
        let mut total_time = n * n;
        //Lowest bound is the starting cell itself
        let mut time = (grid[0][0], total_time);
        while time.0 <= time.1 {
            let expected_time = time.0 + (time.1 - time.0) / 2;
            //Keep searching for the minimal time to get through to the end
            time = if Self::can_swim_through(&grid, n as usize, expected_time) {
                total_time = expected_time;
                (time.0, expected_time - 1)
            } else {
                (expected_time + 1, time.1)
            };
        }
        total_time
    }
...
mod tests {
    ...
    fn test_swim_in_water() {
        for (grid, want) in testcases() {
            assert_eq!(Solution::swim_in_water(grid), want);
        }
    }
}
